...
void enter_room(int room) {
	...
	for // room 1 and room 3

	print_line_sensor();

	shaft=56; // Gives approx shaft count for which folllow line
	...
	if (room!=2) { // if not room 4 and 2 then decide left or right turn

	forward_mm(75); // forward to align the bot on center after
	...
	// after initiating turn, continues till line_conf==10
...
forward(); //start moving forward till ShaftCountLeft <=
...
else { // if room number 4 then no need to go
...
void pickup_service_dumping_section(char current_service) 
{
	...
	int cross, tempv=0, retc=75, shaftc=0;			// shaftc : stores approx shaft count to move
													//retc:shaft count fot returning to service
...
if(current_service=='G') // if service is green turn left else turn right to enter the
