...
const int green_threshold=0;
const int blue_threshold=0;
...
{	
	...
	slow_follow_line(0);
	timer5_init();
	velocity(fl,fr);						// follow initial line
	forward();															
	while(1)
	{
		if(ADC_Conversion(9)>40)					//detect left wall 
		{
			if(current_room==2)
			forward_mm(50);
			forward_mm(60);							//move forward to detect first indicator
			stop();
			break;
		}
	}
	...
	if((int)ADC_Conversion(13)-(int)ADC_Conversion(9)>17 | (int)ADC_Conversion(13)-(int)ADC_Conversion(9)<5)
	{
		back();
		velocity(bl,br);
		print_line_sensor();
	...
	if (judge_order(IA1,IA2)=='E')						// if error in color detection
	{
		...
		judge_order(IA1,IA2);						
		if (current_room==5){
			velocity(fl,fr);						// if last room. Move forward to get in same position.
			forward_mm(620);
		}			
	}
	...
	{	
		...
		servo_3(110);								//rotation angle for color sensor servo motor to close
...
{
	int diff;
	diff = ADC_Conversion(9)-ADC_Conversion(13);
	if(diff>10)
	velocity(255, 250);
	else if(diff < -10)
	velocity (250, 255);
...
void dump_garbage(int room){
	char turn='r';
	int garbage=1;			//dumping garbage will always initiate from cross inside the room i.e. room home
...
void find_line(){
	...
	if(line_conf==0)
	{
		...
		while(ShaftCountRight<12)
		{
			...
			if(line_conf!=0)
			{
				stop();		//if line is detected stop
				return;
			}
		}
		ShaftCountRight=0;
		left();		//if line is not detected go right
		velocity(120,120);
		while(ShaftCountRight<24)		//sweep a larger angle this time
		{
			print_line_sensor();
			if(line_conf!=0)
			{
...
void slow_follow_line(int RqrdLineConf)
{
	...
	{
		if(line_conf==100)
		velocity(50,150);	//move left
		else if(line_conf==110)
		velocity(70,100);	//move left
		else if(line_conf==1)
		velocity(150,50);	//move right
		else if(line_conf==11)
		velocity(100,70);	//move right
		else if (line_conf==10)
		velocity(120,120);	// go straight
		else if(line_conf==111)
		velocity(100,100);	//go straight
		else if(line_conf==0)
		{
		...
		print_line_sensor();
		if (line_conf==RqrdLineConf)	//if required configuration is achived stop
		{
			if (RqrdLineConf==111)
			break;
...
void turn_on_line_slow(char direction){
	...
	while(1)
	if(ADC_Conversion(2)>27)	//if center wl sensor detects black color  break the while loop
...
/*
*
* Function Name: 	follow_line_shaft
* Input: 			distance to go (in terms of shaft count)
* Output: 			void
* Logic: 			Moves forward (high speed) and adusts speed of the wheels so that the bot remains on the line. This is done till the required distance is traversed
...
void follow_line_shaft(int shaftc){
	...
	{
		if(line_conf==100)
		velocity(150,230);//##############was 130,200
		else if(line_conf==110)
		velocity(170,250);//#############was 150,200
		else if(line_conf==1)
		velocity(230,150);//#############was 200,130
		else if(line_conf==11)
		velocity(250,170);//###########was 200,150
		else if (line_conf==10)
		velocity(250,250);//###########was 220,220
		else if(line_conf==111)
		velocity(250,250);//#############was 220,220
		else if(line_conf==0)
		{
...
* Input: 		void
* Output: 		void
* Logic: 		the bot starts from the dumping section and goes to the center of the home 
...
/*
*
* Function Name: 	find_line1
* Input: 			void
* Output: 			void
* Logic: 			Sweeps some area to find the expected line first in the right direction
* Example Call:		find_line1();
*
*/
void find_line1(){
	print_line_sensor();
...
/*
*
* Function Name: 	blink_red/ blink_green/ blink_blue;
* Input: 		void
* Output: 		void
* Logic: 		the led blinks for 1 sec acoording to the specified color
* Example Call:		blink_red
*
*/
void blink_red(){
	PORTL = PORTL & 0x7F;
...
/*
*
* Function Name: 	red_read/ green_read/ blue_read;
* Input: 		void
* Output: 		void
* Logic: 		reads the pulse count of the specified color
...
void follow_line(int RqrdLineConf)
{
	...
	print_line_sensor();
	forward();
	while(1) //the logic inside this loop is similar to that of the slow_follow_linw function
	{
		if(line_conf==100)
		velocity(150,230);//##############was 130,200
		else if(line_conf==110)
...
void back_follow_line_shaft(int shaftc){
	find_line();
	back();
	velocity(200,200);
	int last_line_conf=0;
...
void color_test(){
	color_detect();
	lcd_print(2,1,red,4);
	lcd_print(2,6,green,4);
	lcd_print(2,11,blue,4);
	}
/*
*
...
/*Best of Luck from Deep Vyas.*/
/*thanks from WackySquirrels*/
