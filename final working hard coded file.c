...
const int threshold=600;	//threshold value to decide the color 550 in the night and 800 in the daylight
...
char orders[5]={0,'R','G','R','G'};							//the orders of the rooms 1234 in sequence 2nd position has order room1's order
int sorted_rooms[6]={0,2,4,1,3,0};		//the final sequence of rooms bot has to provide service
int current_room=1;
char position='S';
int count1=1;
...
char color_detect() {
	...
	if(red<threshold && green<threshold && blue<threshold)		// 'K' is for black. When all three pulses are less than threshold value.
	    color = 'K';
...
void take_order1()
{
	...
	forward();										
	timer5_init();	
	if (0/*current_room!=5*/)							//means the room is not the last room
	{	forward_mm(150);
		forward();
		while (1)
		{
			if ((int) ADC_Conversion(6)<30)			// detects wall in front
			{
				stop();
				break;
			}
